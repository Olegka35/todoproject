...
public class OrderServiceImpl implements OrderService {
    ...
    public Order getById(Integer order_id) {
        ...
        for(Integer id: ids) {
            ...
            orderItems.add(new OrderItem(map.get("OI_Type").toString(), Integer.parseInt(map.get("OI_Price").toString()), Integer.parseInt(map.get("OI_Num").toString())));
        }
    ...
    public List<Order> getOrderList(Integer page) {
        List<Map<String, Object>> orders = dao.getList("order", (page-1)* ToDoListConfiguration.TASKS_ON_PAGE, ToDoListConfiguration.TASKS_ON_PAGE);
        List<Order> resultList = new ArrayList<Order>();
        for(Map<String, Object> map: orders) {
            resultList.add(getById(Integer.parseInt(map.get("ID").toString())));
        }
    ...
    public List<Order> getOrderList(Integer page, String order_field, Boolean reversed) {
        List<Map<String, Object>> orders = dao.getList("order", (page-1)* ToDoListConfiguration.TASKS_ON_PAGE, ToDoListConfiguration.TASKS_ON_PAGE, order_field, reversed, null);
        List<Order> resultList = new ArrayList<Order>();
        for(Map<String, Object> map: orders) {
            resultList.add(getById(Integer.parseInt(map.get("ID").toString())));
        }
